...
personOrOrganisation_structure[10] ps;
...
function add_organisation(string memory name, string memory bankacc , string memory bankname) public returns(string memory status){
    ...
    cs[index_organisation].addres = hash;
    cs[index_organisation].name_of_charity = name;
    cs[index_organisation].bankAccount = bankacc;
    cs[index_organisation].bankName = bankname;
